...
\centering
\includegraphics[keepaspectratio,width=0.95\textwidth]{nrmodesLAWE}\label{fig:nrmodesLAWE}
...
L'energia cinetica del modo $E_{kin}^{nl}$ \'e la media temporale di
\begin{equation}
...
\end{equation}
dove $\exv{\dvec{\xi}_{nl}(R)}$ indica la velocit\'a quadratica media superficiale.
...
\end{equation}
che utilizzando \eqref{eq:emper} e \eqref{eq:contper} riscrivo nella forma
...
\begin{equation}
E_{nl}=I_{nl}\exv{V_{nl}^2}=I_{nl}\frac{1}{T_{obs}}\int_{-\infty}^{\infty}|V(\nu)|^2\,d\nu\propto I_{nl}\Gamma_{nl}H_{nl}
\end{equation}
dove $H_{nl}$ \'e l'altezza di picco della densit\'a spettrale di potenza per $V_{l_0m_0}(\nu)$.
...
\begin{wrapfigure}[48]{r}{0.4\textwidth}
\centering
\includegraphics[keepaspectratio,angle=0,width=0.39\textwidth]{rotationshiftridge}
...
\caption{I modi p sono rappresentati da un unica $F$. n \'e l'ordine radiale, $\alpha$ una fase dovuta alla riflessione. Da \cite{duv82dispersion}.}\label{fig:duv82dispersion}
\end{wrapfigure}
