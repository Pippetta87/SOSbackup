...
\begin{workout}[Propriet\'a armoniche sferiche]
...
\begin{workout}[Modi normali acustici: Landau - Cox]
...
\begin{workout}[modello ulrich71-steinliebacher71]
...
\begin{workout}[corotating frame]
...
\begin{workout}[Wave trains propagating east/westward: ridge blue/redshifted.]
...
\begin{wrapfigure}[23]{r}{0.5\textwidth}
\centering
\includegraphics[keepaspectratio,width=0.45\textwidth]{rotationshiftridge}
...
\begin{align}
&\delta\omega_{nlm}\approx m\frac{\int_{r_t}^R\Omega(R)\,\frac{dr}{c}}{\int_{r_t}^R\frac{dr}{c}}&\intertext{per dipendenza puramente radiale, mentre per velocit\'a angolare generale}\\
&\delta\omega_{nlm}\approx m\frac{\int_{\cos{\Theta}}^{-\cos{\Theta}}(\cos^2{\Theta}-\cos^2{\theta})\expy{-\frac{1}{2}}\int_{r_t}^R(1-\frac{L^2c^2}{r^2\omega^2})\expy{\frac{1}{2}}\Omega(r,\theta)\frac{dr}{c}\,d(\cos{\theta})}{\int_{r_t}^R(1-\frac{L^2c^2}{r^2\omega^2})\expy{\frac{1}{2}}\,\frac{dr}{c}}&\intertext{dove}\\
...
\begin{workout}[Bad approximation near surface (no upper turning point) and near solar center]
