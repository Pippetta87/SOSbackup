...
L'inversione del sistema completo di equazioni dei modi si effettua utilizzando un modello di riferimento, per calcolare frequenze e autofunzioni dei modi, 
...
\begin{wrapfigure}[]{r}{0.45\textwidth}\label{fig:soundspeedccm}
        \includegraphics[width=0.44\textwidth,keepaspectratio]{soundspeed}
        \caption{Profilo radiale di $c_s^2$ determinato invertendo \eqref{eq:analinversionc} dalle frequenze dei modi osservate. Da \cite{christensen1985speed}.}
\end{wrapfigure}
...
In assenza di moti macroscopici il termine d'inerzia \'e $\rho_0\TDy{t}{\vec{v}}=\rho_0\PtwoDy{t}{\vec{\xi}}$, mentre in caso di rotazione diventa
\begin{equation}
\rho_0(\PDof{t}+\scap{v_0}{\nabla})^2\vec{\xi}
\end{equation}

Considero la correzione al primo ordine in $\Omega$. Il campo di velocit\'a rotazionale \'e 
\begin{equation}
...
\begin{align}
&\omega_{(l,m)}+\Delta\omega_{(l,m)}\intxt{quindi l'equazione del moto al primo ordine nella perturbazione, con $\alpha=(l,m)$, \'e}
&\rho_0(\omega_{\alpha}^2+2\omega_{\alpha}\Delta\omega_{\alpha})\vec{\xi}=\nabla P_1-\frac{\rho_1}{\rho_0}\nabla P_0+\rho_0\nabla\Phi_1+2i\omega_{\alpha}\rho_0(\scap{v_0}{\nabla})\vec{\xi}\intxt{da cui si deduce, usando $(\scap{v_0}{\nabla})\vec{\xi}=im\vec{\Omega}\xi+\vecp{\Omega}{\xi}$, lo splitting dovuto alla rotazione:}
...
\begin{align}
&\delta\omega_{nlm}\approx m\frac{\int_{r_t}^R\Omega(R)\,\frac{dr}{c}}{\int_{r_t}^R\frac{dr}{c}}\intxt{per dipendenza puramente radiale, mentre per velocit\'a angolare generale}
...
\begin{workout}[inversione rotazione 1D vs 1.5d vs 2d]
...
\begin{align}
&\omega_{nlm}-\omega_{nl0}=m\int_0^R\int_0^{\pi}K_{nlm}(r,\theta)\Omega(r,\theta)r\,dr\,d\theta\intxt{often data doesn't allow determination of individual frequency $\omega_{nlm}$}
&\omega_{nlm}=\omega_{nl0}+2\pi\sum_{j=1}^{j_{max}a_j(n,l)\mathcal{P}_j^{(l)}(m)}\intxt{$\Delta\omega$ e $a_j$ sono legatoi da dipendenza lineare.}
&2\pi a_j(n,l)=\sum_m\gamma_j(l,m)(\omega_{nlm}-\omega_{nl0}),\ 2\pi a_{2j+1}(n,l)=\int_0^R\int_0^{\pi}K_{nlj}^{(a)}(r,\theta)\Omega(r,\theta)r\,dr\,d\theta\intxt{infered angular velocity $\bar{\Omega}(r_0,\theta_0)$ is linearly related to data:}
