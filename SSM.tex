...
\node[anchor=south west] (opint) at ([shift={(0,2)}]current page.south west) {\includegraphics[width=0.55\textwidth,keepaspectratio]{opcontrib-int-g}};
\node[anchor=south east] (opout) at ([shift={(0,2-3/35)}]current page.south east) {\includegraphics[width=0.55\textwidth,keepaspectratio]{opcontrib-out-g}};
\node[draw,anchor=north,label={[label distance=2mm]-90:Scattering \Pphoton\Pelectron},minimum size=5mm] (sc) at ([shift={(0,+7)}]current page.south) {};
...
\node[anchor=north east] at ([shift={(1,3)}]current page.south west) {\parbox{\textwidth}{\captionof{figure}{Importanza dei varii contributi all'opacit\'a nell'interno solare. Da \cite{bla11opacity}.}\label{fig:opacitycontrib} }};
