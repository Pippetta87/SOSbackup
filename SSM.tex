...
\tpc{!h}{
\node at (3.5,0) {\includegraphics[width=0.5\linewidth,keepaspectratio]{HRpulsating} \label{fig:HRp}};
...
\node[anchor=south west] (opint) at ([shift={(0,4)}]current page.south west) {\includegraphics[width=0.55\textwidth,keepaspectratio]{opcontrib-int-g}};
\node[anchor=south east] (opout) at ([shift={(0,4-3/35)}]current page.south east) {\includegraphics[width=0.55\textwidth,keepaspectratio]{opcontrib-out-g}};
...
Illustro in maniera fenomenologica i processi fisici che danno luoga a diffusione , considerando separatamente la diffusione di $\cel{H}{1}{}{}-\cel{He}{4}{}{}$ e degli elementi pesanti. La distribuzione di velocit\'a della specie i \'e approssimativamente la distribuzione di equilibrio traslata di $V_{diff}^i$.

Per la velocit\'a di diffusione relative dei due elementi pi\'u abbondanti, definita la concentrazione relativa $c_i=\frac{n_i}{n}$ e il coefficiente di diffusione $D_{HHe}=-\frac{1}{3}lv_{th}$ con $l\approx(n\sigma)\expy{-1}$

\begin{align}
&v_H-v_{He}=\frac{1}{n_H}\int\,d^3v_Hf_H\vec{v}_H-\frac{1}{n_{He}}\int\,d^3v_{He}f_{He}\vec{v}_{He}\\
...
&\vec{F}_i=\sum_{i\neq j}\vec{F}_{ij},\ \vec{F}_{ij}=m_{ij}n_in_jw_{ij}\vec{v}_{ij}&\intertext{dove la funzione $w_{ij}$ \'e la sezione d'urto di Rutherford per la velocit\'a media relativa delle particelle $i,j$}\nonumber\\
...
&E=-\frac{1}{en_e}\PDy{r}{P_e}&\intertext{Per la velocit\'a di diffusione si ha}\nonumber\\
&v_{HHe}=-\frac{m_{He}T}{m_{HHe}Y\rho w_{HHe}}[\PDy{r}{\ln{(P_eP_H)}}-m_Hg],\ 
n_Hv_H=\frac{(m_{He}n_Hn_{He})}{\rho}v_{HHe}&\intertext{analogamenteper ioni pesanti, con $\cel{H}{1}{}{}$, $\cel{He}{4}{}{}$  elementi pi\'u abbondanti, si trova}\nonumber\\
...
Determino la struttura solare integrando numericamente le equazioni fondamentali della struttura stellare \eqref{subeqn:stellarstructure} per ottenere il profilo radiale delle grandezze $\{P,m,T,L,X_i\}$, note  l'equazione di stato $P(\rho,T,X_i)$, l'opacit\'a $\kappa(P,T,X_i)$, il rate di produzione di energia nucleare per grammo $\epsilon(P,T,X_i)$.
...
e  si tiene conto della diffusione
\begin{equation}
\PDy{t}{n_s}+\frac{1}{r^2}\PDof{r}(r^2n_sw_s)=\Dcvar{\PDy{t}{n_s}}{Nucl}\label{eq:difffusionchange}
\end{equation}
...
\tpc{!h}{
\node at (0,0) {\includegraphics[width=0.9\textwidth,trim=4 4 4 4,clip]{bahcall95}};

}{(0,-7)}{\textwidth}{Profilo radiale}
