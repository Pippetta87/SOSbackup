...
\begin{scope}[name=scopeint]

\node[anchor=north west] (opint) at ([shift={(0,2.5)}]0,0) {\includegraphics[width=0.5\textwidth,keepaspectratio]{opcontrib-int-g}};
...
\begin{scope}[name=scopelegenda,shift={($(scopeint.west) + (0.1,0)$)}]

\node[draw,anchor=north,label={[label distance=2mm]-90:Scattering \Pphoton\Pelectron},minimum size=5mm] (sc) at ([shift={(0,-7)}]10,0) {};
...
\begin{scope}[name=scopeout,shift={($(scopelegenda.west) + (0.1,0)$)}]

\includegraphics[width=0.5\textwidth,keepaspectratio]{opcontrib-out-g}

\end{scope}

\end{tikzpicture}

\caption{Importanza dei varii contributi all'opacit\'a nell'interno solare. Da \cite{bla11opacity}.}\label{fig:opacitycontrib}
