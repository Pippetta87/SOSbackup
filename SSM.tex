...
Per determinare $\exv{\sigma v}$ uso il fatto che l'integrando $S(E)\exp{-\frac{E}{kT}-\frac{b}{\sqrt{E}}}$ ha forma approssimativamente gaussiana il cui massimo $E_G$, energia pi\'u probabile di reazione, e FWHM sono, posto $W=Z_j^2Z_k^2A=Z_j^2Z_k^2\frac{A_iA_j}{A_i+A_j}$:
...
\end{equation}
dove $U(r_{12})\approx0$ per $r\gg r_D$ e $U(r_{12})\to U_0\approx\frac{Z_1Z_2e^2}{r_D}$ per r che tende a zero; dato che $E_G\gg U_0$ approssimo la correzione a \eqref{eq:} con un fattore $f=\exp{-\midfrac{-U_0}{KT}}$.
...
\begin{figure}[!ht]
\begin{subfigure}[r]{0.5\textwidth}\label{fig:dsoundduvall}
...
\begin{subfigure}[r]{0.5\textwidth}
        \includegraphics[width=0.99\textwidth,keepaspectratio]{WrbczHHeioniz}\label{fig:dlessc}
        \caption{Profilo di W calcolato da un modello solare. Da \cite{basu2008helioseismology}.}
    \end{subfigure}
\end{figure}

Il gradiente adimensionale della velocit\'a del suono mette in risalto le rapide variazioni nel profilo termico (alla base della zona convettiva) e in $\Gamma_1$ (nelle zone di ionizzazione)
...
Il buon accordo tra il profilo di $c_s$ determinato eliosismologicamente e calcolato da un modello solare (\subref{fig:dsoundduvall}) giustifica l'uso del modello per calcolare il comportamento dei modi solari che come iluustrer\'o nell'ultima parte permette di determinare il profilo di $\rho$ e $c_s$ in miglior accordo con le osservazioni.

Inoltre ricavando da un modello solare la dipendenza delle quantit\'a determinate eliosismicamente dai parametri del modello \'e possibile imporre dei vincoli ai parametri.
