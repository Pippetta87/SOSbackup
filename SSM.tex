...
Le stelle in sequenza principale, in cui la fusione dei nuclei di \chem{H} in \chem{^4He} bilancia il flusso di energia irradiata verso l'esterno, sono caratterizzate da numerosi modi di oscillazione di piccola ampiezza: onde gravo-acustiche stazionarie le cui caratteristiche sono determinate dal profilo radiale di $P$ e $\rho$, legate dalla condizione di equilibrio idrostatico, dal profilo radiale della velocit\'a del suono e dalla geometria della stella.
...
\'E possibile calcolare numericamente le frequenze di oscillazione attese sulla base di un modello stellare e al variare di uno o pi\'u parametri del modello analizzare la corrispondenza con quelle osservate inoltre sono state sviluppate tecniche per analizzare il problema inverso e valutare le discrepanze modello solare/Sole dalla differenza tra frequenze predette dal modello e osservate.

\begin{figure}[!hb]
    \centering
    \includegraphics[width=0.5\linewidth]{HRpulsating}
    \caption{Stelle pulsanti nel diagramma di \hr{}. Da \cite{chr97lecture}}
    \label{fig:HRp}
\end{figure}

\clearpage

{\let\clearpage\relax \chapter{Strutture autogravitanti in equilibrio}} % Inizio chapter "Tempi scala caratteristici e teorema del viriale ( era Introduzione ai fenomeni oscillatori solari (da vecchia prima parte))" senza nuava pagina
...
\begin{align}
&K=\int_M\frac{1}{\rho}\int n(\vec{p})\frac{p^2}{2m}\,d\vec{p}\,dm=\frac{3}{2}\int_M\frac{P}{\rho}\,dm\\
&K=E_i=\int_0^Mu\,dm
\end{align}
...
Se scrivo $\Omega=-q\frac{GM^2}{R}$, per stelle di sequenza principale ho che $q\approx1.5$: per il modo fondamentale di oscillazione radiale si ha:
\begin{align*}
    &\lambda_1\approx 2\rsun{},\ \omega_1\approx\frac{c_s}{\lambda_1},\ \Pi_1\approx\SI{1}{\hour}
\end{align*}


{\let\clearpage\relax\chapter{Costruzione del modello solare}}%%chapter "Modello solare" senza nuova pagina
...
tenendo conto del contributo cinetico e dell'energia di ionizzazione l'energia interna per unit\'a di massa si scrive
...
\end{equation}
Tiengo conto dell'energia generata sotto forma di neutrini, che, alle densit\'a tipiche dell'interno solare, hanno interazioni trascurabili con la materia e quindi non danno luogo a un flusso di calore nel sistema, aggiungendo un termine negativo $\epsilon_{\nu}$ tale che $L_{\nu}=\int_0^M\epsilon_{\nu}\,dm$.
...
\subsection{Trasporto radiativo dell'energia.}
...
\subsection{Condizione di stabilit\'a dinamica: equilibrio radiativo}
...
\subsection{Mutamenti della composizione chimica: reazioni nucleari e processi di diffusione.}
...
\section{Parametri del modello e condizioni al bordo.}
