...
La correzione alla pressione dovuta alle interazioni coulombiane raggiunge il picco relativo nelle regioni di ionizzazione parziale di idrogeno ed elio, come illustrato da \subref{degenelectrocorrection}.
...
La differenza di pressione calcolata usando l'equazione di stato OPAL e MHD \'e $\midfrac{\Delta P}{P}\leq1\%$  per $\frac{r}{\rsun{}}\geq0.9$ e $\midfrac{\Delta P}{P}\leq0.2\permille$ per $\frac{r}{\rsun{}}\leq0.9$, mentre $\midfrac{\Delta \Gamma_1}{\Gamma_1}\leq1\%$  per $\frac{r}{\rsun{}}\geq0.9$ e $\midfrac{\Delta \Gamma_1}{\Gamma_1}\leq1\permille$ per $\frac{r}{\rsun{}}\leq0.9$.
...
\end{equation}
con $E_i$ \'e l'energia interna dovuta ai moti traslazionali \eqref{eq:traslintenergy} e $I=\int r^2\,dm$. In condizioni stazionarie $\frac{1}{2}\TtwoDy{t}{I}\approx0$:
\begin{equation}
...
Tengo conto dell'energia generata sotto forma di neutrini, che, alle densit\'a tipiche dell'interno solare, hanno interazioni trascurabili con la materia e quindi dal punto di vista energetico sono paragonabili a un flusso di energia verso uscente: aggiungo un termine negativo $-\epsilon_{\nu}$ tale che $L_{\nu}=\int_0^M\epsilon_{\nu}\,dm$.
...
\end{align}
 dove ho definito la concentrazione relativa $c_i=\frac{n_i}{n}$, il coefficiente di diffusione $D_{12}=\frac{1}{3}lv_{th}$ con $l\approx(n\sigma)\expy{-1}$ cammino libero medio e il coefficiente di diffusione termica $K_T$.
...
Scrivo il termine collisionale $C(f)$, considerando solo il contributo degli urti tra la specie j con distribuzione $f_j$ , nella forma
\begin{equation}
...
Considero il problema in cui le due specie hanno velocit\'a relativa media diversa da zero ma piccola rispetto alla velocit\'a termica: nel sistema in cui la prima specie ha velocit\'a media nulla la seconda ha velocit\'a $V$ quindi la distribuzione di velocit\'a della prima \'e la distribuzione di equilibrio a temperatura T quella della seconda \'e  la distribuzione di equilibrio a temperatura T traslata di $V_{ij}$, velocit\'a di diffusione:
\begin{equation}
...
La forza di archimede ha verso opposta alla perturbazione se $[\Dcvar{\TDy{r}{\rho}}{e}-\Dcvar{\TDy{r}{\rho}}{amb}]$ .
...
L e stelle con massa $M\leq1.1\msun{}$ hanno una regione radiativa interna mentre la parte esterna \'e convettiva: la regione in cui vale l'uguaglianza in \eqref{eq:ledoux}, la base della zona convettiva, la cui posizione \'e determinata dall'aumentare dell'opacit\'a col diminuire della temperatura.
...
\end{equation}
eApprossimando il gradiente normale alla superficie con $\exv{\Delta T}$ ed usando le definizioni \eqref{eq:nablavitense} si ottiene
\begin{equation}
...
\begin{wrapfigure}[16]{r}{0.4\textwidth}
       ...
       \caption{Andamento della correzione di schermaggio degli elettroni secondo diversi schemi.}
\end{wrapfigure}
...
\end{equation}
dove $U(r_{12})\approx0$ per $r\gg r_D$ e $U(r_{12})\to U_0\approx\frac{Z_1Z_2e^2}{r_D}$ per r che tende a zero; dato che $E_G\gg U_0$ approssimo la correzione a \eqref{eq:fusioncrosssection} con il fattore moltiplicativo
\begin{equation}
...
La modellizzazione della fisica del plasma solare (EOS, opacit\'a,) sono necessarie approssimazioni che ne influenzano l'accuratezza, mentre per quanto riguarda la composizione chimica le misure pi\'u accurate sono quelle basate sulle righe di assorbimento dell'atmosfera solare e quelle basate sulla composizione dei meteoriti (condriti CI), che si suppone abbiano conservato composizione originaria a parte H,C,N,O e gas nobili (i due metodi sono in accordo per gli altri elementi): le composizioni basate su una modellizzazione pi\'u accurata dell'atmosfera solare e della fisica atomica portano ad una metallicit\'a solare pi\'u bassa (vedi \ref{tab:Zhistory}).
...
\begin{subfigure}[r]{0.5\textwidth}
        ...
        \caption{Differenza relativa del profilo di $c_s$ determinata invertendo \eqref{eq:analinversionc} per frequenze dei modi calcolate da un modello e osservate. Da \cite{christensen1985speed}.}
\label{dsoundduvall}
    \end{subfigure}
~
\begin{subfigure}[r]{0.5\textwidth}
...
\begin{wrapfigure}[15]{r}{0.3\textwidth}
        \includegraphics[width=0.3\textwidth,keepaspectratio]{deltaCdeltaQ}
        \caption{Profilo della derivata logaritmica di $c_s$ rispetto ai parametri del MSS $(\PDly{Q}{c_s})$.}
\end{wrapfigure}

Il buon accordo tra il profilo di $c_s$ determinato eliosismologicamente e calcolato da un modello solare (\subref{dsoundduvall}) giustifica l'uso del modello per calcolare il comportamento dei modi solari che come iluustrer\'o nell'ultima parte permette di determinare il profilo di $\rho$ e $c_s$ in miglior accordo con le osservazioni.
