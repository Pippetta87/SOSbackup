...
\phantomcaption
\end{subfigure}
\caption{A sinistra (a): Parametro di degenerazione $\Psi$ e correzioni alla pressione dovute alla degenerazione degli elettroni nell'interno solare. A destra (b): Rapporto fra energia interna traslazionale e coulombiana la prima e fra pressione e correzione coulombiana la seconda.}
\end{figure}
...
Il potenziale $V_i(r)$ dovuto allo ione $Z_i$ altera la distribuzione di cariche quindi devo determinare il potenziale $\phi_i$ generato da $Z_i$ e dalla distribuzione di cariche attorno: per la formula di Boltzmann, la densit\'a delle particelle con carica Z \'e 
\begin{equation}
n_Z=\overline{n}_Z\exp{-\frac{Ze\phi_i}{kT}}
\end{equation}
con $\overline{n}_Z$ densit\'a numerica della particell $Z$ in assenza di $Z_i$. Assumendo l'energia coulombiana molto minore dell'energia termica espando $n_Z$ al prim'ordine quindi l'equazione di Poisson per $\phi$ \'e
\begin{equation}
...
\end{equation}
quindi ho l'equazione di Poisson per il potenziale a distanza $|\vec{r}-\vec{r}_i|$ dallo ione $Z_i$ in $\vec{r}_i$:
