...
\begin{equation}
u=\frac{1}{\rho}\int n(\vec{p})\frac{p^2}{2m}=\frac{3}{2}\frac{P}{\rho}=\frac{3}{2}\frac{\gasconstant T}{\mu}
\end{equation}

dove $n(\vec{p})$ \'e il numero di particelle per unit\'a di volume con impulso in $[\vec{p},\vec{p}+d\vec{p}]$ e u \'e l'energia interna per unit\'a di massa, quindi l'energia interna dell'intera stella \'e 
\begin{equation}
...
\item Ionizzazione atomica stati eccitati.

\begin{workout}[regioni di ionizzazione:]
...
In figura \ref{gamma1eos} si vede che $\Gamma_1\approx\midfrac{5}{3}$ nell'interno solare e maggiori deviazioni si hanno nelle regioni di ionizzazione parziale degli elementi in particolare di idrogeno ed elio.

\begin{figure}[!h]
\begin{subfigure}[r]{0.5\textwidth}
        \includegraphics[width=0.95\textwidth,keepaspectratio]{ionfraction}
    ...
    \end{subfigure}
~
\begin{subfigure}[r]{0.95\textwidth}
        \includegraphics[width=0.5\textwidth,keepaspectratio]{gamma1eos}
        \caption{Andamento di $\Gamma_1$. Da \cite{trampedach2006synoptic}}
    \end{subfigure}
\end{figure}
...
La principale correzione che tiene conto dell'interazioni tra particelle \'e dovuta alle interazioni coulombiane e come illustrato da \subref{fig:RatioelectroEP} raggiunge il picco nelle regioni di ionizzazione parziale di idrogeno ed elio.
...
Importante nella regione di ionizzazione di H e He.
...
Per i valori \ref{fig:sunO} ottengo valori di circa $27$ min.: quindi la costanza delle caratteristiche solari su tempi molto maggiori giustifica l'ipotesi di equilibrio idrostatico, quindi  riscrivo il tempo scala di evoluzione dinamica come
...
&C(f,f_1)=\int\,d^3p_1\,d\sigma|\vec{v}-\vec{v}_1|(f'f_1-ff_1)\intxt{Considero la forza netta dovuta agli urti:}
...
&\vec{R}=nn_1\mu\alpha\vec{V},\ \alpha=\frac{\mu}{KT}\int u^3\sigma^Tf^{(0)}(\vec{v_r})\,d^3v_r,\ \sigma^T=\int(1-\cos{\chi})\,d\sigma
\end{align}

\begin{workout}[stima coefficiente di diffusione/segno]

Per la velocit\'a di diffusione relative dei due elementi pi\'u abbondanti, idrofeno ed elio, definita la concentrazione relativa $c_i=\frac{n_i}{n}$ e il coefficiente di diffusione $D_{HHe}=\frac{1}{3}lv_{th}$(dubbio) con $l\approx(n\sigma)\expy{-1}$ cammino libero medio
...
il primo termine tende a diminuire il gradiente di concentrazione, il secondo tiene conto della forza risultante dal disequilibrio tra gradiente di pressione parziale e forza di gravit\'a e il terzo della forza risultante su un nucleo pesante dovuta all'
...
Considero il plasma solare costituito da idrogeno, elio ed elettroni; il momento trasferito dagli ioni agli elettroni,$q\approx\mu|\vec{v}-\vec{v}_1|\chi$, \'e piccolo quindi si ha
...
&w_{HHe}=-\frac{m_{He}T}{m_{HHe}Y\rho \alpha'_{HHe}}[\PDy{r}{\ln{(P_eP_H)}}-m_Hg],\ 
...
{\let\clearpage\relax\let\cleardoublepage\relax
\chapter{Modello solare standard e incertezze. Osservabili sismologiche.}
}
...
\end{equation}
\end{subequations}
con $v_i$ velocit\'a di diffusione specie i. Ottengo il profilo radiale delle grandezze $\{P,m,T,L,X_i\}$, note  l'equazione di stato $P(\rho,T,X_i)$, l'opacit\'a $\kappa(P,T,X_i)$, il rate di produzione di energia nucleare per grammo $\epsilon(P,T,X_i)$.
...
Per determinare il modello solare attuale evolvo il modello del Sole in sequenza principale partendo una composizione chimica fissata fino ad ottenere un modello con le caratteristiche del Sole attuale:
\begin{itemize}
...
\section{Parametri fisici del modello}
...
\begin{workout}[EOS: incertezze sistematiche MHD-OPAL]
...
La differenza di pressione calcolata usando l'equazione di stato OPAL e MHD: $\midfrac{\Delta P}{P}\leq1\%$  per $\frac{r}{\rsun{}}\geq0.9$, $\midfrac{\Delta P}{P}\leq0.2\%$ $\frac{r}{\rsun{}}\leq0.9$

\end{workout}


\begin{workout}[opacit\'a solare e sua incertezza]
...
\begin{equation}
\delta\kappa(r)=\frac{\kappa_{OPAL}(\bar{T}(r),\bar{\rho}(r),\bar{Y}(r),\bar{Z}(r))}{\kappa_{OP}(\bar{T}(r),\bar{\rho}(r),\bar{Y}(r),\bar{Z}(r))}-1
\end{equation}

$\delta\kappa\leq0.025$.

\begin{tikzpicture}[]

\node[anchor=north west] (opint) at ([shift={(1.5,0)}]0,0) {\includegraphics[width=0.45\textwidth,keepaspectratio]{opcontrib-int-g}};
\node[anchor=north east] (opout) at ([shift={(1.5,0-3/35)}]0,0) {\includegraphics[width=0.45\textwidth,keepaspectratio]{opcontrib-out-g}};
...
\node[anchor=north west] at ([shift={(0,-12.5)}]0,0) {\parbox{\textwidth}{\captionof{figure}{Importanza dei varii contributi all'opacit\'a nell'interno solare. Da \cite{bla11opacity}.}\label{fig:opacitycontrib} }};
...
\captionof{table}{Metallicit\'a osservata. Da \cite{asplund2009chemical}.}
\end{table}
...
\begin{tablenotes}
\item[1]$R(\cel{Be}{7}{}{}(\Pelectron,\Pnue)\cel{Li}{7}{}{})=5.6(1\pm0.02)10^{-9}\midfrac{\rho}{\mu_e}T_6\expy{-1/2}[1+0.004(T_6-16)]\si{\per\second}$ ; \item[2] $R(p\Pelectron p)=1.102(1\pm0.01)10^{-4}\midfrac{\rho}{\mu_e}T_6\expy{-1/2}[1+0.002(T_6-16)]R(pp)$
\end{tablenotes}
